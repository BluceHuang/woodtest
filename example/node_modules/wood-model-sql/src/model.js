...
class Model {
  ...
  // 更新数据
  async update(data = {}, addLock = false) {
    const { catchErr, error } = this.ctx;
    ...
    if (lock.data) {
      const result = await catchErr(this.db.update(this.tableName. this.getData()));
      if(addLock) this.redis.unlock(lock.data);
      if (result.data){
  ...
  async findOneAndUpdate(data = {}, addLock = true) {
    return this.update(data, addLock);
  }
      ...
      if (!hasLock.data) {
        if (hasCache) {
          ...
          if (hasKey) {
            ...
            let result = await this.db.find(this.tableName, params);
            if (result.err) {
              throw error(result.err);
            } else {
              return result.data;
        ...
        const docsResult = await this.db.find(this.tableName, params, largelimit);
        if (docsResult.err) {
